...
Input: parameters for choose_background_and_query (q3 stays False if MS1-only) 
...
"""
def MethodProfiler(compounds_filt, spectra_filt,change = 0.7, ppm = 0, change_q3 = 0.7, ppm_q3 = 0, col_energy = 35, col_gas = '',
    ...
    profiled = profile(change = change, ppm = ppm, change_q3 = change_q3, ppm_q3 = ppm_q3, col_energy = 35, col_gas = '',
                       ion_mode = ion_mode, inst_type = inst_type, adduct = adduct, q3 = True, top_n = 0.1, mol_id = mol_id, compounds_filt = compounds_filt, spectra_filt = spectra_filt, UIS_num = UIS_num)
...
def collision_energy_optimizer():
    ...
    collision_energy = []
    isotope_num = []
    copy = compounds_filt#.sample(n=1200, random_state=1)
    spectra_filt = spectra_filt.loc[spectra_filt['mol_id'].isin(list(copy.mol_id))]
    collision_all = []

    for i, molecule in copy.iterrows():
        molid = molecule['mol_id']
        query, background, frag_mz, frag_mz_int = choose_background_and_query(mol_id = molid, col_energy = 0, change=25, q3 = False, spectra_filt = spectra_filt, inst_type = ['Q-TOF'],choose=False)
    ...
    copy.to_csv("CE_opt_1223_qtof_25da.csv")
    return copy
